  ...
  jQuery(function() {
    $.tags = function(element, options) {
      ...
      this.makeSuggestions = function(e, overrideLengthCheck) {
		...
		if (_this.caseInsensitive) { val = val.toLowerCase();}
        _this.suggestedIndex = -1;
        _this.$suggestionList.html('');
        _this.suggestionList = [];
        $.each(_this.suggestions, function(i, suggestion) {
		  suggestionval = _this.caseInsensitive ?  suggestion.substring(0, val.length).toLowerCase() :  suggestion.substring(0, val.length);
          if (_this.tagsArray.indexOf(suggestion) < 0 && suggestionval === val && (val.length > 0 || overrideLengthCheck)) {
